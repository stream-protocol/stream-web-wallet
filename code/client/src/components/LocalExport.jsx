...
const LocalExport = ({ wallet }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = `${wallet.name.toLowerCase().split(' ').join('-')}-${oneAddress}.StreamWallet`
  ...
  const showExportModal = () => {
    Modal.confirm({
      content: <Text><Paragraph>The exported file is meant for cross-device transfer. Please do not keep the exported file on your device for longer than necessary. </Paragraph><Paragraph>Doing so would make your wallet less secure, and opens up possibilities for an attacker to hack your wallet.</Paragraph><Paragraph>For more technical details, please read <Link href='https://github.com/stream-protocol/stream-web-wallet/wiki/Client-Security' target='_blank' rel='noreferrer'>Client Security in StreamWallet wiki</Link></Paragraph></Text>,
      onOk: handleExport,
    })
  }
