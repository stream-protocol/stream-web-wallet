...
export const parseOAuthOTP = (url) => {
  ...
  try {
    ...
    if (issuer !== 'Stream Wallet' && issuer !== 'Harmony' && issuer !== 'StreamWallet') {
      message.error('Invalid issuer of the recovery QR code. Expecting `StreamWallet` or `Harmony`')
      return null
    }
...
export const parseMigrationPayload = url => {
  ...
  if (filteredParams.length === 2) {
    ...
    if (names[0] !== names[1]) {
      message.error('You selected two StreamWallets with different names. If you want to select two entries belonging to the same wallet, make sure they have the same name and the second one has "- 2nd" in the end')
      return undefined
    }
