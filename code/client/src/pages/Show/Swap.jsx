...
const Swap = ({ address }) => {
  ...
  const buildSwapOptions = (tokens, setSelectedToken) => tokens.map((token, index) => {
    ...
    const outer = token.disabled ? <Tooltip title='This option is temporarily disabled. It will be available in the future version of StreamWallet'>{inner}</Tooltip> : inner
    ...
    <>
      ...
      {unknownError &&
        <TallRow>
          <Col span={24}>
            <Warning>
              An unknown error occurred. Network might be offline. The token pair may have insufficient liquidity. Please submit a bug report if you think you caught a bug in StreamWallet. Error: {unknownError}
            </Warning>
          </Col>
        </TallRow>}
