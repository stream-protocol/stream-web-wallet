...
const PrepareWalletSection = ({ expertMode, showRecovery, coreSettings, setupConfig, walletState, setWalletState, progress, progressStage, network }) => {
  ...
  const deploy = async () => {
    ...
    try {
      ...
      handleAPIError(ex)
      message.error('Failed to deploy StreamWallet. Please try again. If it keeps happening, please report this issue.')
      setDeploying(false)
      setDeployed(false)
    }
  }
    ...
    <AnimatedSection>
      <Row>
        <Space direction='vertical'>
          <Heading>Prepare Your StreamWallet</Heading>
        </Space>
      </Row>
      {expertMode &&
        ...
        <Row style={{ marginBottom: 24 }}>
          {!showRecoveryDetail &&
            <Space>
              ...
              <Tooltip title={'It is where you could send your money to if you lost the authenticator. You don\'t have to configure this. By default it goes to StreamWallet DAO'}>
          ...
          {showRecoveryDetail &&
            <Space direction='vertical' size='small' style={{ width: '100%' }}>
              ...
              <Hint>
                ...
                If you lost access, you can still send your assets there or use <Link href='https://github.com/stream-protocol/stream-web-wallet/releases/tag/v0.2' target='_blank' rel='noreferrer'>auto-recovery</Link>
              </Hint>
              {util.isDefaultRecoveryAddress(lastResortAddress.value) &&
                <Warning style={{ marginTop: 24 }}>
                  StreamWallet DAO can be a last resort to recover your assets. You can also use your own address.
                </Warning>}
            </Space>}
        </Row>}
      <Row style={{ marginBottom: 32 }}>
        <Space direction='vertical'>
          ...
          {!showRecovery &&
            <TallRow>
              {(deploying || !coreSettings.root) && <Space><Text>Working on your StreamWallet...</Text><LoadingOutlined /></Space>}
              {(!deploying && coreSettings.root && deployed) && <Text>Your StreamWallet is ready!</Text>}
              {(!deploying && coreSettings.root && deployed === false) && <Text>There was an issue deploying your StreamWallet. <Button type='link' onClick={() => (location.href = Paths.create)}>Try again</Button>?</Text>}
            </TallRow>}
      ...
      <Row>
        <Space direction='vertical'>
          ...
          <Hint>To learn more, visit <Link href='https://github.com/stream-protocol/stream-web-wallet/wiki'>StreamWallet Wiki</Link></Hint>
        </Space>
      </Row>
    </AnimatedSection>
  )
    ...
    return (
      <AnimatedSection2 SectionEl={Card} bodyStyle={{ padding: 0 }}>
        <Space direction='vertical' style={{ width: '100%', padding: '32px 32px 16px 32px' }}>
          {step === 1 &&
            <>
              <Heading level={isMobile ? 4 : 2}>Create Your StreamWallet</Heading>
          ...
          {step === 2 &&
            <>
              <Heading>Create Your StreamWallet (second code)</Heading>
