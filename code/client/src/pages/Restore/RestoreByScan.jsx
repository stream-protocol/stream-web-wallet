...
const RestoreByScan = ({ isActive, onComplete, onCancel }) => {
  ...
  const onRestore = (ignoreDoubleOtp) => {
    ...
    try {
      ...
      if (identificationKeys.filter(e => e.length >= 130).length > 0) {
        ...
        if (idKeyIndex === -1) {
          message.error('Seed QR code does not match StreamWallet identification key at the address. If this is unexpected, please report this bug to us.')
          return
        }
      }
  ...
  return (
    <Space direction='vertical' size='large'>
      <Heading>Restore StreamWallet by Scanning</Heading>
      ...
      {secret && address &&
        <>
          ...
          <WalletCreateProgress progress={progress} isMobile={isMobile} progressStage={progressStage} subtitle='Rebuilding your StreamWallet' />
        </>}
