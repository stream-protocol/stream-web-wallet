...
const computeMerkleTree = async({
    ...
    const eotps = await hasher(input, {
        progressObserver: buildProgressObserver(n * maxOperationsPerInterval * 3 - 1, 1)
    })
    const leaves = await sha256b(eotps, {
        progressObserver: buildProgressObserver(n * maxOperationsPerInterval * 3 - 1, 1, n * maxOperationsPerInterval)
    })
    ...
    if (buildInnerTrees) {
        ...
        const interlaced = sha256Interlaced(otps, {
            progressObserver: buildProgressObserver(totalNumOps, 2),
            unitSize: 4,
            window: 6
        })
...
const computeRecoveryHash = ({ randomSeed, hseed }) => {
    randomSeed = randomSeed || new Uint8Array(new BigUint64Array([0 n, BigInt(Date.now())]).buffer)
...
/**
 * DEPRECATED: This is DEPRECATED as Client Security is already implemented. https://github.com/stream-protocol/stream-web-wallet/wiki/Client-Security
 * @param hseed
 * @param nonce
 * @param leaf
